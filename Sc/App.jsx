...
const CUSTOMER_ID_MAP_KEY = "kwuo-temp-customer-id-map";

function loadCustomerIdMap() {
  try {
    return JSON.parse(localStorage.getItem(CUSTOMER_ID_MAP_KEY) || "{}");
  } catch {
    return {};
  }
}

function saveCustomerIdMap(map) {
  try {
    localStorage.setItem(CUSTOMER_ID_MAP_KEY, JSON.stringify(map));
  } catch {}
}
...
function overlayPendingCustomers(list, q, idMap = {}) {
  const existingIds = new Set(list.map((c) => c.id));
  const extra = q
    .filter((i) => i.type === "addCustomer" && !existingIds.has(i.tempId) && !existingIds.has(idMap[i.tempId]))
    .map((i) => ({
      id: i.tempId,
      name: i.name,
      phone: i.phone,
      createdAt: i.queuedAt || Date.now(),
      pending: true,
    }));
  return [...list, ...extra];
}

function overlayPendingTxns(list, q, idMap = {}) {
  ...
  const extra = q
    ...
    .map((r) => {
      const mappedRow = { ...r, customer_id: idMap[r.customer_id] || r.customer_id };
      return { ...rowToTxn(mappedRow), pending: true };
    });
  return [...list, ...extra];
}
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const q = loadQueue();
      if (cachedCustomers) setCustomers(overlayPendingCustomers(JSON.parse(cachedCustomers), q, idMap));
      if (cachedTxns) setTxns(overlayPendingTxns(JSON.parse(cachedTxns), q, idMap));
    } catch {}
    ...
    try {
      ...
      const q = loadQueue();
      const currentMap = loadCustomerIdMap();
      const loadedCustomers = overlayPendingCustomers((custRows || []).map(rowToCustomer), q, currentMap);
      const loadedTxns = overlayPendingTxns((txnRows || []).map(rowToTxn), q, currentMap);
      ...
      // network slow/unreachable — cached data stands as-is
    }
  }, [business]);
  ...
  const enqueue = useCallback((action) => {
    setQueue((q) => {
      const next = [
        ...
        {
          ...action,
          qid: action.qid || (crypto.randomUUID ? crypto.randomUUID() : String(Date.now() + Math.random())),
  ...
  const flushQueue = useCallback(async () => {
    ...
    async function syncAddCustomer(item) {
    ...
    async function syncInsertTxns(rows) {
      const { error } = await supabase.from("transactions").insert(rows);
      if (!error) return true;

      const isDuplicate = error.code === "23505" || (error.message && error.message.includes("23505"));
      if (isDuplicate) {
        ...
        if (!checkErr && existing && existing.length === ids.length) {
          return true;
        }
        return false;
      }
      ...
      while (q.length > 0) {
        ...
        for (let i = 0; i < q.length; i++) {
          ...
          if (item.type === "addCustomer") {
            ...
            const unmappedTempCid = item.rows
              .map((r) => r.customer_id)
              .find((cid) => typeof cid === "string" && cid.startsWith("temp-") && !idMap[cid]);

            if (unmappedTempCid) {
              const depIndex = q.findIndex(
                (it) => it.type === "addCustomer" && it.tempId === unmappedTempCid
              );
              if (depIndex !== -1) {
                targetIndex = depIndex;
                break;
              } else {
                continue;
              }
        ...
        const item = q[targetIndex];

        if (item.type === "addCustomer") {
          if (idMap[item.tempId]) {
            q = q.map((qi) => {
              if (qi.type === "insertTxns") {
                return {
                  ...qi,
                  rows: qi.rows.map((r) => ({
                    ...r,
                    customer_id: r.customer_id === item.tempId ? idMap[item.tempId] : (idMap[r.customer_id] || r.customer_id),
                  })),
                };
              }
              return qi;
          ...
          try {
            const data = await syncAddCustomer(item);
            if (data) {
              idMap[item.tempId] = data.id;
              saveCustomerIdMap(idMap);
              ...
              q = q.map((qi) => {
                if (qi.type === "insertTxns") {
                  return {
                    ...
                    rows: qi.rows.map((r) => ({
                      ...r,
                      customer_id: r.customer_id === item.tempId ? data.id : (idMap[r.customer_id] || r.customer_id),
                    })),
                  };
                }
                return qi;
          ...
          const rowsToInsert = item.rows.map((r) => ({
            ...r,
            customer_id: idMap[r.customer_id] || r.customer_id,
          }));

          const stillHasTemp = rowsToInsert.some(
            (r) => typeof r.customer_id === "string" && r.customer_id.startsWith("temp-")
          );
          ...
          try {
            const success = await syncInsertTxns(rowsToInsert);
            if (success) {
              q.splice(targetIndex, 1);
              saveQueue(q);
              changed = true;
  ...
  async function insertTxnRows(rows) {
    ...
    const hasTempCustomer = withIds.some(
      (r) => typeof r.customer_id === "string" && r.customer_id.startsWith("temp-")
    );

    if (!hasTempCustomer && navigator.onLine) {
      try {
  ...
  async function addCustomer(name, phone) {
    ...
    const tempId = `temp-${crypto.randomUUID ? crypto.randomUUID() : Date.now() + "-" + Math.random().toString(36).slice(2)}`;
